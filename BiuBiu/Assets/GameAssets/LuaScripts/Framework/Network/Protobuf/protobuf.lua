...
local pb = require "pb"
local wire_format = require "Framework.Network.Protobuf.wire_format"
local type_checkers = require "Framework.Network.Protobuf.type_checkers"
local encoder = require "Framework.Network.Protobuf.encoder"
local decoder = require "Framework.Network.Protobuf.decoder"
local listener_mod = require "Framework.Network.Protobuf.listener"
local containers = require "Framework.Network.Protobuf.containers"
local descriptor = require "Framework.Network.Protobuf.descriptor"
local FieldDescriptor = descriptor.FieldDescriptor
local text_format = require "Framework.Network.Protobuf.text_format"
local _M = {}
...
local _ED_meta = {
    ...
    __newindex = function(self, extension_handle, value)
        local _extended_message = rawget(self, "_extended_message")
        if
            (extension_handle.label == FieldDescriptor.LABEL_REPEATED or
                extension_handle.cpp_type == FieldDescriptor.CPPTYPE_MESSAGE)
         then
            error(
                'Cannot assign to extension "' ..
...
local function _AddPropertiesForFields(descriptor, message_meta)
    ...
    if descriptor.is_extendable then
        message_meta._getter.Extensions = function(self)
...
local function _AddStaticMethods(message_meta)
    message_meta._member.RegisterExtension = function(extension_handle)
        ...
        if message_meta._extensions_by_number[extension_handle.number] == nil then
            ...
            error(
                string.format(
                    'Extensions "%s" and "%s" both try to extend message type "%s" with field number %d.',
                    extension_handle.full_name,
                    actual_handle.full_name,
                    message_meta._descriptor.full_name,
...
local function _AddIsInitializedMethod(message_descriptor, message_meta)
    ...
    message_meta._member.IsInitialized = function(self, errors)
        for _, field in ipairs(required_fields) do
            if
                self._fields[field] == nil or
                    (field.cpp_type == FieldDescriptor.CPPTYPE_MESSAGE and not self._fields[field]._is_present_in_parent)
             then
                if errors ~= nil then
